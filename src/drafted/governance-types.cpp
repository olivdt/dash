...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "NewCoinNetwork") type = NewCoinNetwork;
    if(strType == "NewCoinNetworkVariable") type = NewCoinNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == NewCoinNetwork) s = "NewCoinNetwork";
    if(type == NewCoinNetworkVariable) s = "NewCoinNetworkVariable";
