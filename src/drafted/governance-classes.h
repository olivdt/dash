 ...
 *
 *   // network
 *   CNewCoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.newcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CNewCoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CNewCoinNetwork(UniValue objIn)
    {
...
// // can be under: NewCoinNetwork
// //   -- signature requirements : Key1(User)
// class CNewCoinNetworkVariable : public CGovernanceObject
// {
// private:
