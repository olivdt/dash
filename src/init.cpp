...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "newcoin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all NewCoin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N NEWCOIN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-19284, default: %u)"), DEFAULT_INSTANTSEND_DEPTH));
...
/** Sanity checks
 *  Ensure that NewCoin Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ...
        mapArgs["-privatesendrounds"] = "192849";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendrounds=192849\n", __func__, nLiqProvTmp);
        mapArgs["-privatesendamount"] = "1928499";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendamount=1928499\n", __func__, nLiqProvTmp);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("NewCoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize NewCoin Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NewCoin Core is shutting down."));
...
#endif
    // Make sure only a single NewCoin Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NewCoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NewCoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of NewCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart NewCoin Core to complete") << "\n";
    ...
    privateSendClient.nPrivateSendRounds = std::min(std::max((int)GetArg("-privatesendrounds", DEFAULT_PRIVATESEND_ROUNDS), 2), privateSendClient.nLiquidityProvider ? 192849 : 16);
    privateSendClient.nPrivateSendAmount = std::min(std::max((int)GetArg("-privatesendamount", DEFAULT_PRIVATESEND_AMOUNT), 2), 1928499);
#endif // ENABLE_WALLET
    ...
    // ********************************************************* Step 11c: update block tip in NewCoin modules
    ...
    // ********************************************************* Step 11d: start newcoin-ps-<smth> threads
