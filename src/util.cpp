...
const char * const BITCOIN_CONF_FILENAME = "newcoin.conf";
const char * const BITCOIN_PID_FILENAME = "newcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "newcoin" is a composite category enabling all NewCoin-related debug output
            if(ptrCategory->count(string("newcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NewCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NewCoinCore
    // Mac: ~/Library/Application Support/NewCoinCore
    // Unix: ~/.newcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NewCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NewCoinCore";
#else
    // Unix
    return pathRet / ".newcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty newcoin.conf if it does not excist
